...
Das Requirements Engineering erfolgt entwicklungsbegleitend, wobei die Stakeholder fortlaufend einbezogen werden. Auf diese Weise wird eine kontinuierliche Überprüfung und Anpassung der Anforderungen sichergestellt.
...
FDD folgt einem klar strukturierten Prozess, der in fünf Phasen gegliedert ist. Diese Phasen lassen sich in zwei Bereiche einteilen: die Aufgaben des Planungsteams und die der Entwicklungsteams.
...
Anschließend werden jedem Entwicklungsteam ein oder mehrere Features zugeordnet.
Die Teams erarbeitet zunächst ein Design-Paket für die jeweiligen Features (\emph{Phase 4: Design by Feature}), anschließend erfolgt die Implementierung (\emph{Phase 5: Build by Feature}).\cite{coad99}
...
Trotz dieser Gemeinsamkeiten gibt es wesentliche Unterschiede zwischen beiden Konzepten:
...
In \cite{palmer02} wird zudem betont, dass ein Feature nicht nur eine triviale Accessor-Methode sein sollte, die lediglich einen Attributwert zurückgibt.
...
Während Objekte und Klassen aus User Stories abgeleitet werden können, werden Features typischerweise aus der bestehenden Softwarearchitektur bzw. dem Ablaufmodell abgeleitet \cite{palmer02}.
Dadurch sind User Stories im Entwicklungsprozess vor Features einzuordnen.

Features und User Stories lassen sich also nicht miteinander vergleichen, während eine User Story ein oder mehrere Features umfassen kann, kann ein einzelnen Feature auch in mehreren User Stories vorkommen\cite{coad99}.

% Beispiel fehlt noch

\subsubsection{Rolle von User Stories in FDD}
